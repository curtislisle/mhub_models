...
class Patho_RMS_Runner_Binary(ModelRunner):
    #fractional : bool
    #batchsize: int
...
def _inference(model, image_path, BATCH_SIZE, num_classes, kernel, num_tta=1):
    ...
    # convert from probability map to binary labelmap image by evaluating which class has the highest probability
    pred_map_final = np.argmax(prob_map_seg_stack, axis=-1)
    del prob_map_seg_stack
    gc.collect()
    # each channel is either 0 or 1 as a binary image, no scaling to 255 needed, just convert to integers
    out_label = (pred_map_final).astype('uint8')
    return out_label
...
def writeDicomSegObject(image_path, seg_image, out_path):
    ...
    print(f'writeDCM: received seg image stack of shape {seg_image.shape}')
    ...
    print('passing in a numpy array of shape:',mask.shape)
    

    # Describe the algorithm that created the segmentation
    algorithm_identification = hd.AlgorithmIdentificationSequence(
        name='FNLCR_IVG_RMS_seg_iou_0.7343_epoch_60_fold_03',
        version='v1.0',
    ...
    # change output file with some function here, if needed
